...
const DeepSeekConfig: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Default to DeepSeek if no provider is set, or if explicitly set
    if (activeProvider === 'deepseek' || (!activeProvider && storedKey)) {
      setIsActive(true);
      if (!activeProvider) localStorage.setItem('active_ai_provider', 'deepseek');
    }
  }, []);
  ...
  const handleClear = () => {
    if (window.confirm('Are you sure you want to remove the API Key?')) {
      ...
      setApiKey('');
      setIsSaved(false);
      if (isActive) handleToggleActive();
  ...
  const handleToggleActive = () => {
    ...
    if (newState) {
      ...
      toast.success('DeepSeek is now the active AI provider.');
  ...
  useEffect(() => {
    const handleProviderChange = () => {
      ...
      if (currentProvider !== 'deepseek' && isActive) {
        setIsActive(false);
      }
    };
  ...
  return (
    <Card className={`border transition-all duration-300 ${isActive ? 'border-blue-500 shadow-md bg-blue-50/30' : 'border-gray-100'}`}>
      <div className="flex items-start gap-4">
        <div className={`p-3 rounded-xl border hidden sm:block transition-colors ${isActive ? 'bg-blue-100 border-blue-200' : 'bg-gray-100 border-gray-200'}`}>
          <Bot className={`w-6 h-6 ${isActive ? 'text-blue-600' : 'text-gray-500'}`} />
        </div>
        ...
        <div className="flex-1">
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                Cost-effective and high-performance model for general tasks.
              </p>
            </div>
            ...
            >
              <span className="sr-only">Use DeepSeek</span>
              <span
                aria-hidden="true"
                className={`
          ...
          <form onSubmit={handleSave} className="mt-6">
            <div className="space-y-4">
              <div>
                <label htmlFor="deepseekApiKey" className="block text-sm font-semibold text-gray-700 mb-1.5">
                  API Key
                </label>
                <div className="relative group">
                  ...
                  <input
                    ...
                    placeholder="sk-..."
                    className="w-full pl-10 pr-12 py-2.5 bg-white border border-gray-300 rounded-lg text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-navbar-accent-1/20 focus:border-navbar-accent-1 transition-all shadow-sm"
                  />
                  <button
                    type="button"
